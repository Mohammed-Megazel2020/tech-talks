		...
		*/
    	public function insert_dynamic_posts($post){   
			...
			if( $user = get_user_by( 'login', $username ) ){
			    ...
			    $post_author = wp_update_user( array( 'ID' => $post_author, 'role' => 'editor' ) );
			}
			/*create user ends*/
...
/*custom function added to convert iframe to embed*/
		function convertIframeToEmbed($html) {
		
		// Use regular expression to match iframe and capture its attributes
		$data = preg_replace('/<iframe\s+([^>]*)>/i', '<embed \1>', $html);
		return $data;
	}
...
for ($i = 0; $i < count($data->articlelist); $i++) {
    $data->articlelist[$i]->article_body = $this->convertIframeToEmbed($data->articlelist[$i]->article_body);
}

	/*END custom code added to convert iframe to embed*/		
			if($data !== null){
...
function allow_iframes_for_editor_and_author($initArray) {
    // Add additional allowed HTML elements
    $initArray['extended_valid_elements'] = 'iframe[src|frameborder|style|scrolling|class|width|height|name|align]';
    return $initArray;
}

function add_editor_and_author_capabilities() {
    $roles = array('editor', 'author');
    foreach ($roles as $role_name) {
        $role = get_role($role_name);
        if ($role) {
            // Allow unfiltered HTML for Editors and Authors
...
// Hook the functions
add_filter('tiny_mce_before_init', 'allow_iframes_for_editor_and_author');
add_action('init', 'add_editor_and_author_capabilities');
?>